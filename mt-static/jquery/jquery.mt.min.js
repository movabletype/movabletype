...
.click(function(event){return false;});};$.mtSelector=function(options){var defaults={arrow_image:StaticURI+'images/arrow/arrow-toggle.png'};var opts=$.extend(defaults,options);$('#system-overview > em').prepend('<a hre="#" class="toggle-button"><img src="'+opts.arrow_image+'" /></a>');$('#user-dashboard > em').prepend('<a href="#" class="toggle-button"><img src="'+opts.arrow_image+'" /></a>');$('#current-website > em').prepend('<a href="#" class="toggle-button"><img src="'+opts.arrow_image+'" /></a>');$('#selector-nav-list li .toggle-button ')
.mousedown(function(event){$(this).parent('em').parents('#selector-nav').toggleClass('show-selector active');return false;})
.click(function(event){return false;});$(document).mousedown(function(event){if($(event.target).parents('#selector-nav').length==0){$('#selector-nav').removeClass('show-selector active');}});if(!$.support.style&&!$.support.objectAll){if($.fn.bgiframe)$('div.selector').bgiframe();}};$.mtCMSSearch=function(options){$('#cms-search > a')
.mousedown(function(event){$('#cms-search').toggleClass('active').find('#search-form').toggleClass('hidden');return false;})
.click(function(event){return false;});$(document).mousedown(function(event){if($(event.target).parents('#cms-search').length==0){$('#search-form').addClass('hidden').parent('#cms-search').removeClass('active');}});};$.mtUseSubdomain=function(options){var $checkboxes=$('.url-field :checkbox');$checkboxes.each(function(){if(!this.checked){$(this).parents('.field-content').find('.subdomain').hide();}});$checkboxes.click(function(){if(this.checked){$(this).attr('checked',true).parents('.field-content').find('.subdomain').show();}else{$(this).removeAttr('checked').parents('.field-content').find('.subdomain').hide();}});};$.mtEditSiteUrl=function(options){var defaults={edit:'Edit'};var opts=$.extend(defaults,options);var ids=['site','archive'];$.each(ids,function(){var id=this;var $subdomain=$('input#'+this+'_url_subdomain');var $path=$('input#'+this+'_url_path');var subdomain=$subdomain.val();if(subdomain){$subdomain
.parent('.subdomain')
...
$.data(this,'mtValidateLastError',msg);$this.addClass(validator.errorClass);$this.removeClass(validator.validClass);}});return errors==0;},mtUnvalidate:function(){this.each(function(){var validator=$(this).mtValidator();if(validator){var $current_error=$.data(this,'mtValidateError');if($current_error){validator.removeError($(this),$current_error);}
$.data(this,'mtValidator',null);$.data(this,'mtValidateError',null);$.data(this,'mtValidateLastError',null);$(this).removeClass(validator.errorClass);$(this).removeClass(validator.validClass);}});return this;}});$('input, textarea').live('keyup focusin focusout',function(){var ns=$.data(this,'mtValidator');if(!ns)return true;$(this).mtValid();});})(jQuery);