...
.appendTo(form);});form.trigger('submit');form.remove();}
else{$('<iframe />')
...
.css('color','GrayText')
.attr('data-showing-placeholder','showing');}})
.parents('form').on('submit',function(){if($that.attr('data-showing-placeholder')){$that.prop('data-showing-placeholder');$that.val('');}});});return this;};$.fn.mtEditInputBlock=function(options){var defaults={edit:'Edit',text:'Not specified'},opts=$.extend(defaults,options);return this.each(function(){var id=$(this).attr('id'),$div=$('div#'+id),$date=$('input#'+id),$time=$('input#'+id);if(!$div.hasClass('show-input'), false){$div
.before('<span class="'+id+'-text"></span>')
...
$('button#mt-edit-'+id).on('click',function(){$(this).hide();$('span.'+id+'-text').hide();$div.show();return false;});});};})(jQuery);
