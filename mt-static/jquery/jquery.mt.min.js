...
$('.mt-dialog').show();$('.mt-dialog').bind('close',function(event,fn){fn(event);});$('.mt-dialog-overlay').show();if(opts.esckeyclose){$(document).keyup(function(event){if(event.keyCode==27){close_dialog();}});}
...
return new $.mtValidator(ns,options);}};$.extend($.mtValidator.prototype,{options:{},validateElement:function($elem,val){var validator=this;var rules=$.mtValidateRules;validator.error=false;validator.errstr=undefined;$.each(rules,function(selector,fn){if($elem.is(selector)){validator.error=false;validator.errstr=undefined;validator.test=fn;var res=validator.test($elem);if(validator.error||!res){if(!validator.errstr){validator.raise($.mtValidateMessages['#'+$elem.attr('id')+selector]||$.mtValidateMessages[selector]||'Invalid value');}
return false;}}});return!validator.error;},raise:function(msg){this.error=true;this.errstr=this.trans(msg||'Invalid value');return false;},trans:function(msg){return('function'==typeof window.trans)?trans(msg):msg;},validClass:'valid',errorClass:'error',wrapError:function($target,msg){return $('<div />').append($('<label/>')
