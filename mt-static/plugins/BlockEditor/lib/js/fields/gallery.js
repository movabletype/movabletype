; (function ($) {
    ...
    BEF.Gallery = function () { BEF.apply(this, arguments) };
    BEF.Gallery.createButton = function () {
        return $('<span class="add">Gallery</span>');
    };
    $.extend(BEF.Gallery.prototype, BEF.prototype, {
        id: '',
        input_field: '',
        options: {},
        ...
        create: function (id, data) {
            var self = this;
            self.id = id;
            self.input_field = $('<div class="asset_field"><a href="' + ScriptURI + '?__mode=dialog_list_asset&amp;edit_field=' + id + '&amp;blog_id=' + $('[name=blog_id]').val() + '&amp;dialog_view=1&amp;filter=class&amp;filter_val=image&amp;no_insert=1&amp;next_mode=block_editor_asset&amp;can_multi=1" class="mt-open-dialog">edit Image</a><input type="hidden" name="' + id + '" id="' + id + '" value=""></div>');
        ...
        get_data: function () {
            ...
            return {
                ...
                'preview_html': $(self.get_html()).html(),
                'html': self.get_html(),
        ...
        get_html: function () {
            ...
            if (self.class_names.length > 0) {
                html += ' ' + this.class_names.join(' ');
            }
            html += '">' + self.preview_field.html() + '</div>';
            return html;
        }
    });

    MT.BlockEditorFieldManager.register('gallery', BEF.Gallery);
