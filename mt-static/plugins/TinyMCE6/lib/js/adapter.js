                ...
                strikethrough: [
                    {
                        ...
                        onformat: function (elm, fmt, vars) {
                            ...
                            var datetime =
                                [now.getFullYear(), z(now.getMonth() + 1), z(now.getDate())].join('-') +
                                'T' +
                                [z(now.getHours()), z(now.getMinutes()), z(now.getSeconds())].join(':') +
                                m[1] +
                                m[2] +
                                ':' +
                                m[3]
    ...
    $.extend(MT.Editor.TinyMCE.prototype, MT.Editor.prototype, {
        initEditor: function (format) {
            ...
            if (adapter.commonOptions['content_css_list'].length > 0) {
                config['content_css'] = ((config['content_css'] ? config['content_css'] + ',' : '') + adapter.commonOptions['content_css_list'].join(','))
                    .replace(/^,+|,+$/g, '')
                    .replace(/"/g, '&qquot;')
            }
            config['body_class'] = config['body_class'] + ' ' + adapter.commonOptions['body_class_list'].join(' ')
        ...
        setFormat: function (format, calledInInit) {
            ...
            if (mode == 'source') {
                if (this.editor !== this.source) {
                    this.$editorTextarea.insertAfter(this.$editorIframe).height('auto')
        ...
        insertContent: function (value) {
            if (this.editor === this.source) {
                ...
                if (node && node.nodeName === 'IMG') {
                    ...
                    selection.isCollapsed = function () {
                        return true
                    }
        ...
        _init_instance_callback: function (ed) {
            ...
            adapter.source = new MT.Editor.Source(adapter.$editorTextarea.attr('id'))
            adapter.proxies = {
            ...
            var Cookie = tinymce.util.MovableTypeCookie
            var size = Cookie.getHash('TinyMCE_' + ed.id + '_size')
            if (size) ed.resizeTo(size.cw, size.ch)
            ...
            $(['SetContent', 'KeyDown', 'Reset', 'Paste', 'Undo', 'Redo']).each(function () {
                var ev = this
                ed.on(ev, function () {
