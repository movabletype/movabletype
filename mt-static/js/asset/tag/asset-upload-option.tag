    ...
    <div if={parent.opts.filter != 'userpic'}>
      <div class="row" if={ parent.opts.allow_to_change_at_upload }>
        <div class="col row" if={ parent.opts.destination_loop }>
          ...
          <input type="text" id="upload_destination_custom" class="form-control text path required valid-custom-path upload-destination w-100 col-12" disabled="disabled" name="destination" value={ parent.upload_options.destination } style="display: none;" aria-describedby="uploadDestination" onchange={ changeDestinationText }>
          <div class="col">
            <div class="upload-extra-path row">
              <div class="col-auto h5 mt-auto mb-auto">/</div>
              <div class="col w-100 ml-0 pl-0"><input type="text" name="extra_path" id="extra_path" class="form-control text path w-100 ml-0" value={ parent.upload_options.extra_path } onkeyup={ changeExtraPath } ></div>
            </div>
          </div>
        </div>
      </div>
  ...
  <fieldset class="form-group">
    ...
    <div class="form-group">
      <div class="custom-control custom-checkbox">
        <input type="checkbox" name="auto_rename_non_ascii" id="auto_rename_non_ascii" value="1" checked={ parent.upload_options.auto_rename_non_ascii } class="cb custom-control-input" onchange={ changeAutoRenameNonAscii }>
    ...
    <select name="operation_if_exists" id="operation_if_exists" class="custom-select form-control text-truncate" onchange={ changeOperationIfExists }>
      <option value="1" selected={ parent.upload_options.operation_if_exists == 1 }>
        { trans('Upload and rename') }
      </option>
      <option value="2" selected={ parent.upload_options.operation_if_exists == 2 }>
        { trans('Overwrite existing file') }
      </option>
      <option value="3" selected={ parent.upload_options.operation_if_exists == 3 }>
        { trans('Cancel upload') }
      </option>
    </select>
    <div class="form-group">
      <div class="custom-control custom-checkbox">
        <input type="checkbox" name="normalize_orientation" id="normalize_orientation" value="1" checked={ parent.upload_options.normalize_orientation } class="cb custom-control-input" onchange={ changeNormalizeOrientation }>
