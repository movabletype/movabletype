...
export const svelteMountBreadcrumbsButton = ({ target }: { target: Element }): void => {
    const dataBreadcrumbs = target.getAttribute("data-breadcrumbs");
    const dataScopeType = target.getAttribute("data-scope-type");
    const dataCanAccessToSystemDashboard = target.getAttribute("data-can-access-to-system-dashboard");
    const dataCanCurrentWebsiteLink = target.getAttribute("data-curr-website-can-link");
