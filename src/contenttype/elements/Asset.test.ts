...
describe("Asset Component", () => {
  it("should render the component", () => {
    ...
    const { container } = render(Asset, { props });
  ...
  it("should render labels for options", () => {
    ...
    expect(
      labelTexts.some((text) => text?.includes("upload a new asset")),
    ).toBe(true);
  });
  ...
  it("should show min/max fields when multiple checkbox is checked", async () => {
    ...
    const minField = container.querySelector("#asset-min-field");
    const maxField = container.querySelector("#asset-max-field");
