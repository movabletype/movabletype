    ...
    if (droppable) {
      ...
      if (dragged) {
        ...
        if (e.target.className === 'mt-draggable__area') {
          ...
          if (fields.length === 0 || ( fields.length === 1 && fields[0] === dragged)){
  ...
  function onDrop(e) {
    ...
    if (dragged) {
      ...
      for (var i = 0; i < children.length; i++){
        ...
        if (children[i] != dragged && children[i].classList.contains("mt-contentfield")) {
          pos++;
        }
      }
...
//    }
  ...
  function rebuildLabelFields() {
    ...
    for (var i = 0; i < $cfields.length; i++) {
      var required = jQuery('#content-field-block-' + $cfields[i].id).find('[name="required"]').prop('checked');
      if (required && $cfields[i].canDataLabel === 1) {
        var label = $cfields[i].label;
        var id = $cfields[i].unique_id;
        if (!label) {
          label = jQuery('#content-field-block-' + $cfields[i].id).find('[name="label"]').val();
          if (label === '') {
            label = trans('No Name');
          }
          id =  'id:' + $cfields[i].id;
  ...
  function _validateFields() {
    ...
    const res = requiredFieldsAreValid
              ...
              <div class="modal-body">
                <div class="col">
                  <div id="name-field" class="form-group">
                    ...
                    <input type="text" name="name" id="name" class="form-control html5-form" value={ opts.name } on:keypress={ stopSubmitting } required>
                  </div>
                </div>
                <div class="col">
                ...
                <div class="col">
                  <div id="label-field" class="form-group">
                    ...
                    <select id="label_field" name="label_field" class="custom-select form-control html5-form" onchange={ changeLabelField }>
                      ...
                      <option value="{ l.value }" selected="{ l.value === parent.labelField }">{ l.label }</option>
                      {/each}
                    </select>
                  </div>
                </div>
...
<form>
  <fieldset id="content-fields" class="form-group">
    ...
    <div class="mt-draggable__area" style="height:400px;" on:drop={ onDrop } on:dragover={ onDragOver } on:dragleave={ onDragLeave }>
      {#if isEmpty}
      ...
      <div id="content-field-block-{f.id}" class="mt-contentfield" draggable="true" aria-grabbed="false" on:dragstart={ onDragStart } on:dragend={ onDragEnd } style="width: 100%;">
...
<button type="button" class="btn btn-primary" disabled={ !canSubmit() } on:click={ submit }>{ trans("Save") }</button>
