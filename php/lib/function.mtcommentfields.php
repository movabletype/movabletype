...
function smarty_function_mtcommentfields($args, &$ctx) {
    ...
    $comment_script = $this->mt->config('CommentScript');
    $e =& $ctx->stash('entry');
    if (!$e) {
    ...
    $typekey_version = $this->mt->config('TypeKeyVersion');
    ...
    $tk_version = $this->mt->config('TypeKeyVersion');
