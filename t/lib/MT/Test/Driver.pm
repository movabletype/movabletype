...
## Test saving created object
    ...
    is( $foo->column('id'), 1,
        'First Foo was given an id of 1, says column()' );
...
## created Bar object. Make sure they are unique.
    ...
    are_objects( \@tmp, \@foo,
        'unique Foos associated with Bars, oldest first' );
...
## so this should load foo[0]
    ...
    are_objects(
        \@tmp,
        [ $foo[0] ],
        'One Foo associated with Bars of status=0'
    );
...
## and load_iter.
    ...
    are_objects( \@tmp, \@foo,
        'Foos loaded by explicit join across columns' );
...
## TEST EXISTS METHOD
    ...
    $tmp->id(5);
    ok( !$tmp->exists,
        'New Foo just created with fake id does not exist in db' );
