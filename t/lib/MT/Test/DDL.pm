...
sub _note { note( MT::Util::Encode::encode_if_flagged($enc, shift) ) }
...
sub _01_create_table : Tests(2) {
    ...
    for my $create_sql (@create_sql) {
        ...
        MT::Test::DDL::_note( $dbh->errstr || $DBI::errstr ) if !$res;
    }
}
...
sub _02_create_indexes : Tests(6) {
    ...
    for my $index_sql (@index_sql) {
        ...
        if ( !$res ) {
            MT::Test::DDL::_note( $dbh->errstr || $DBI::errstr );
            MT::Test::DDL::_note( 'SQL: ' . $index_sql );
        }
    }
}
...
sub is_def {
    ...
    if ( !defined $got_type || $expected_type ne $got_type ) {
        fail($reason);
        MT::Test::DDL::_note( "Expected db data type ",
    ...
    if ( defined $expected->{size} && $expected->{size} != $got->{size} ) {
        fail($reason);
        MT::Test::DDL::_note( "Expected size ", $expected->{size}, " but got ",
            $got->{size} );
        return;
    }
...
sub multikey_unique : Tests(1) {
    ...
    MT::Test::DDL::_note( 'Saving error: ', $err ) if !$ret;
}
...
SKIP: {
    ...
    SQL: for my $sql (@sql) {
            $res = $dbh->do($sql);
            MT::Test::DDL::_note( ( $dbh->errstr || $DBI::errstr ) . "( sql $sql)" ) if !$res;
            last SQL if !$res;
        }
