...
MT->set_instance($app);
$app->user( $app->model('author')->anonymous );
...
for my $d ( glob( File::Spec->catfile( $spec_dir, '*' ) ) ) {
    ...
    $app->config->DisableDataAPIResourceCache(0);
    subtest "$model: Cached" => $runner;
    $app->config->DisableDataAPIResourceCache(1);
    subtest "$model: Not cached" => $runner;
}
...
sub from_object {
    ...
    for my $d (@$suite) {
        ...
        my $hash
            ...
            : MT::DataAPI::Resource->from_object_with_cache($obj);
