...
for my $module ( qw/ Log4perl Minimal / ) {
    for my $level( qw/ debug warn / ) {
        SKIP: {
            ...
            eval { MT::Util::Log::_find_module( $level, $path, $module ) };
            skip "Log::$module class isn't installed.", 8 if $@;
            ...
            if ( $level eq 'debug' ) {
                ok ( $fmgr->exists($logfile_path), 'A data was written to a file.' );
            }
            else {
                ok ( $fmgr->exists($logfile_path), 'A data was not written to a file.' );
            }
        }
    }
}
