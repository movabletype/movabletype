...
$test_env->prepare_fixture(sub {
    ...
    my $tb_entry = MT::Test::Permission->make_tb(
        blog_id        => $blog->id,
        entry_id       => $entry->id,
        title          => 'my entry trackback',
    );
    my $tb_page = MT::Test::Permission->make_tb(
        blog_id        => $blog->id,
        entry_id       => $page->id,
        title          => 'my page trackback',
    );
...
my $kemigawa = MT::Author->load( { name => 'kemigawa' } );

my $admin = MT::Author->load(1);

my $entry    = MT::Entry->load( { title => 'my entry' } );
my $tb_entry = MT::Trackback->load( { title => 'my entry trackback' } );
my $tb_page  = MT::Trackback->load( { title => 'my page trackback' } );

my $ping_entry = MT::TBPing->load( { title => 'my entry ping' } );
my $ping_page  = MT::TBPing->load( { title => 'my page ping' } );
