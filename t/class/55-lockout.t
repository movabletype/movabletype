...
$author_class->remove( { name => [ 'Evil', 'Good', $unknown_name ], } );
...
ok( !MT::Lockout->is_locked_out_ip( $app, $evil_ip_address ),
    '$evil_ip_address has not locked out. (FailedLogin: 0)'
);
ok( !MT::Lockout->is_locked_out_ip( $app, $good_ip_address ),
...
ok( !MT::Lockout->is_locked_out_user( $app, $evil_user->name ),
    '$evil_user has not locked out. (FailedLogin: 0)'
);
ok( !MT::Lockout->is_locked_out_user( $app, $good_user->name ),
    ...
    )
{
    ...
    is( MT::Lockout->is_locked_out() ? 1 : 0,
        $case->{res},
        sprintf(
...
{
    $fixed_time = $now - 10;
    MT::Lockout->_insert_failedlogin( $app, $good_ip_address,
        $good_user->name );
    MT::Lockout->_insert_failedlogin( $app, $evil_ip_address,
        $good_user->name );
    MT::Lockout->_insert_failedlogin( $app, $evil_ip_address,
    ...
    MT::Lockout->process_login_result( $app, $case->{remote_ip},
        $case->{user}->name,
        MT::Auth::NEW_LOGIN(), );
    is( $app->model('failedlogin')->count,
        1,
...
is( $app->model('failedlogin')->count,
    1, 'Failedlogin has 1 entries. (cleaned)' );
...
clear_lockout_statuses();
{
    ...
    ok( !$evil_user->locked_out, '$evil_user is never locked out.' );
}
...
is( $task->{frequency},
    $app->config->FailedLoginExpirationFrequency,
    "task's frequency: is FailedLoginExpirationFrequency"
);
$task->{code}->();
is( $app->model('failedlogin')->count,
    0, 'Failedlogin has 0 entries. (cleaned)' );

note('Callbasks');

note('pre_lockout / post_lockout');
my (@pre_lockout_args,       @pre_lockout_user_args,
    @pre_lockout_ip_args,    @post_lockout_args,
    @post_lockout_user_args, @post_lockout_ip_args,
);
...
is( $pre_lockout_args[0][2],
    $unknown_name, 'pre_lockout was passed username' );
is( $pre_lockout_args[0][3], $evil_ip_address, 'pre_lockout was passed ip' );

is( scalar @post_lockout_args, 1, 'post_lockout ran once.' );
is( $post_lockout_args[0][2],
    $unknown_name, 'post_lockout was passed username' );
is( $post_lockout_args[0][3], $evil_ip_address,
    'post_lockout was passed ip' );

is( scalar @pre_lockout_ip_args, 1, 'pre_lockout.ip ran once' );
is( $pre_lockout_args[0][2],
    $unknown_name, 'pre_lockout.ip was passed username' );
is( $pre_lockout_args[0][3],
    $evil_ip_address, 'pre_lockout.ip was passed ip' );

is( scalar @post_lockout_ip_args, 1, 'post_lockout.ip ran once' );
is( $post_lockout_args[0][2],
    $unknown_name, 'post_lockout.ip was passed username' );
is( $post_lockout_args[0][3],
    $evil_ip_address, 'post_lockout.ip was passed ip' );
...
(   @pre_lockout_args,       @pre_lockout_user_args,
    @pre_lockout_ip_args,    @post_lockout_args,
    @post_lockout_user_args, @post_lockout_ip_args,
) = ();
clear_lockout_statuses();
...
is( scalar @pre_lockout_args, 1, 'pre_lockout ran once' );
is( $pre_lockout_args[0][2],
    $evil_user->name, 'pre_lockout was passed username' );
is( $pre_lockout_args[0][3], $evil_ip_address, 'pre_lockout was passed ip' );

is( scalar @post_lockout_args, 1, 'post_lockout ran once.' );
is( $post_lockout_args[0][2],
    $evil_user->name, 'post_lockout was passed username' );
is( $post_lockout_args[0][3], $evil_ip_address,
    'post_lockout was passed ip' );

is( scalar @pre_lockout_user_args, 1, 'pre_lockout.user ran once' );
is( $pre_lockout_args[0][2],
    $evil_user->name, 'pre_lockout.user was passed username' );
is( $pre_lockout_args[0][3],
    $evil_ip_address, 'pre_lockout.user was passed ip' );

is( scalar @post_lockout_user_args, 1, 'post_lockout.user ran once' );
is( $post_lockout_args[0][2],
    $evil_user->name, 'post_lockout.user was passed username' );
is( $post_lockout_args[0][3],
    $evil_ip_address, 'post_lockout.user was passed ip' );
