...
is( $MT::Serialize::SERIALIZER_VERSION, 2,
    'Default serializer version is 2' );
...
for (qw/ MTS Storable /) {
    isnt(
        ...
        "Serializer version of $_ is not "
            . $MT::Serialize::SERIALIZER_VERSION,
    );
}
...
for my $label ( sort keys %sers ) {
    ...
    is( $ser->serializer_version($frozen),
        $ser_version,
        'Returns correct serializer version '
            . ( defined $ser_version ? $ser_version : '"undef"' ),
    );
...
SKIP: {
        ...
        is( ${ $thawed->[1]{c}[2] },
            3, 'Returns correct value for HASH{c} 3/3' );
...
for my $label (qw(MT2 MTJ MTS)) {
    ...
    if ( $label eq 'MT2' || $label eq 'MT' ) {
        $_ += 0
            for $thawed->[0], $thawed->[1]{b}[0], $thawed->[1]{c}[2],
