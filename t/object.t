...
my $count = $mt->model('entry')->count({class => 'entry'});
...
$count = $mt->model('entry')->count({id => {not => '0'}, class => "*"});
is($count, $entry_count+$page_count);

$count = $mt->model('entry')->count({id => {not => '0'}, class => "entry"});
is($count, $entry_count);

$count = $mt->model('entry')->count({id => {not => '0'}, class => "page"});
is($count, $page_count);
