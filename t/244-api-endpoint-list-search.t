...
my $author = MT->model('author')->load(4);
        ...
        items        => [
            map {
                $format->{unserialize}->( $format->{serialize}
                        ->( MT::DataAPI::Resource->from_object($_) ) )
...
my @suite = (
    (map {
        +{  path   => '/v1/sites/1/entries',
            method => 'GET',
            params => {
                searchConditions  => MT::Util::to_json([
                    {
                        type => 'id',
                        args => {
                            value => $_,
    ...
    {   path   => '/v1/sites/1/entries',
        note   => 'A conditions that is never matching',
        method => 'GET',
        params => {
            searchConditions  => MT::Util::to_json([
                {
