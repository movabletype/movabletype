...
my $mt = new MT( Config => 't/mt.cfg' );

require 't/blog-common.pl';
...
XML
    ...
    ok(Date::Parse::str2time($atom_obj->issued()),
...

{
    ...
    require Data::Dumper;
# print Data::Dumper::Dumper($atom_entry);
    ok($atom_entry->title(), "Fight the Power");
    ok($atom_entry->author()->get('name'), $username);
    ok($atom_entry->content()->body(),
    ...
    if (ok($resp->is_success)) {
        ...
        ok($atom_entry->title, "Fight the Power");
        ok(Date::Parse::str2time($atom_entry->issued), 
