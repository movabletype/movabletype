...
$blog->touch;
$blog->save;
is( scalar( keys %$is_changed ),
    0, q{$blog->touch doesn't change any meta value} );
...
is( $blog2->captcha_provider, q{foo},
    q{Captcha provider meta value saved successfully} );
is( scalar( keys %$is_changed ),
    1, q{$blog->captcha_provider update touches one meta value} );
...
ok( (           $blog3->captcha_provider eq q{bar}
            and $blog3->update_pings eq q{baz}
            and $blog3->include_system eq q{zap}
    ),
...
is( scalar( keys %$is_changed ),
    3,
    q{Multiple meta value saves correctly identify number of saved meta columns}
);
