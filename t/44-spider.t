...
BEGIN {
    plan $ENV{MT_TEST_SPIDER}
        ? ( tests => 4 )
        : ( skip_all =>
            'Enable spider test with MT_TEST_SPIDER environment variable' );
}
...
my $start_link
    = "http://localhost"
    . $cgipath
    . "/mt.cgi?username=$username&password=$password";
...
while ( keys %link_queue && $count < 500 ) {
    ...
    unless ( $links_checked{$curr_link} ) {
        ...
        print STDERR $resp->content()
            unless $resp->content() =~ m/$good_pattern/;
