...

{
...

    {
        ...
        is( ( $asset->thumbnail_file( Height => 100, Width => 100 ) )[0],
            $test_env->path("site/assets_c/$cache_path/test-thumb-100xauto-1.jpg"),
            'thumbnail file name'
        );
        my ( $width, $height )
            = imgsize(
...

    {
        ...
        is( ( $asset->thumbnail_file( Height => 100, Square => 1 ) )[0],
            $test_env->path("site/assets_c/$cache_path/test-thumb-100x100-1.jpg"),
            'thumbnail file name'
        );
        my ( $width, $height )
            = imgsize($test_env->path("site/assets_c/$cache_path/test-thumb-100x100-1.jpg"));
        is( $width,  100, "resized image's width: 100" );
    ...
    my $orig_file
        = File::Spec->catfile( $ENV{MT_HOME}, "t", 'images', 'test.jpg' );
    my $copy_file
        = File::Spec->catfile( $test_env->root, 'test_.jpg' );
    ...
    my $copy_file_f = File::Spec->catfile( $test_env->root, 'test_.tmpl' );
    copy( $orig_file, $copy_file );
