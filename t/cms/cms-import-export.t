...
my $app = _run_app( 'MT::App::CMS',
    { __test_user => $user, __mode => 'export', blog_id => $blog->id } );
...
eval {
    $app
        = _run_app( 'MT::App::CMS',
        { __test_user => $user, __mode => 'export', blog_id => 1000 } );
};
my $eval_error = $@;
ok($eval_error);
...
ok( MT::Entry->count( { blog_id => $blog->id } ) == 0,
    "Got rid of all entries" );

my @comments = MT::Comment->load( { blog_id => $blog->id } );
foreach my $comment (@comments) {
    $comment->remove;
}
ok( MT::Comment->count( { blog_id => $blog->id } ) == 0,
...
close IN;
my $iter = $impt->_get_stream_iterator( 'test_import.txt',
    sub { my ($string) = @_; print STDERR "[cb_iterator] $string\n"; 1 } );
