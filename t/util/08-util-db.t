...

{
    ...
    ok( $entry = get_entry( $timestamp, 1, 'Daily', 'previous' ),
        'get_entry() with "previous"' );
    is( $entry ? $entry->title : '',
        'Verse 5', 'result of get_entry() with "previous"' );

    ok( !get_entry( $timestamp, 1, 'Category' ),
        'get_entry() with invalid archive_type'
    );
}

{
    my $iter1 = $mt->model('blog')
        ->load_iter( undef, { sort => 'id', limit => 1 } );
    my $iter2 = $mt->model('entry')
        ->load_iter( undef, { sort => 'id', limit => 1 } );
    my $iter3 = $mt->model('comment')
        ->load_iter( undef, { sort => 'id', limit => 1 } );
