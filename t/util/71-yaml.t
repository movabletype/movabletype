...
SKIP: {
        ...
        my $data;
        ok( ($data) = MT::Util::YAML::LoadFile('t/util/71-yaml.yaml'),
            "$class: Loadfile" );
        ok( 'HASH' eq ref $data,     "$class: returns HASHREF" );
        ok( defined $data->{TITANS}, "$class: exists key 'TITANS'" )
            and
            is( scalar @{ $data->{TITANS} }, 2, "$class: number of array" );
        is( $data->{AEUG}{"Hyaku Shiki"},
            "Quattro Bajeena",
            "$class: get hash value"
        );
        ok( Encode::is_utf8( $data->{AEUG}{Methuss} ),
            "$class: wide character is utf8"
        );
        my $str;
