...
my $objs = $test_env->prepare_fixture(
    sub {
        ...
        my $cd_admin = MT::Test::Permission->make_content_data(
            ...
            convert_breaks => MT::Serialize->serialize(
                \{ $content_field_2->id => 'markdown' }
            ),
        );
...
subtest 'authorized fields' => sub {
    ...
    ok( $out !~ /permission=1/, 'edit by aikawa' );
    ...
    is( $saved_cd->data->{ $content_field_2->id }, 'test text2',
        'test field2' );
    my $convert_breaks =
      MT::Serialize->unserialize( $saved_cd->convert_breaks );
    is( $$convert_breaks->{ $content_field_2->id },
