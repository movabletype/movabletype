...
my $tmpl_ct_archive = MT::Test::Permission->make_template(
    blog_id         => $blog_id,
    content_type_id => $ct->id,
    name            => 'ContentType Test ct_archive',
    type            => 'ct_archive',
    text            => 'test',
);
my $map_ct_category_monthly = MT::Test::Permission->make_templatemap(
...
subtest 'create content_data' => sub {
    ...
    $app->post_ok({
        __mode                              => 'save',
        _type                               => 'content_data',
        return_args                         => "__mode=view&blog_id=$blog_id&type=content_data_$ct_id&_type=content_data&content_type_id=$ct_id",
        blog_id                             => $blog_id,
...
subtest 'update content_data (change date_and_time field)' => sub {
    my $cd1 = MT->model('content_data')->load({ blog_id => $blog_id, label => 'cd1' })
        or die;
    my $app = MT::Test::App->new('MT::App::CMS');
    $app->login($author);
    $app->post_ok({
        __mode                              => 'save',
        _type                               => 'content_data',
        return_args                         => "__mode=view&blog_id=$blog_id&type=content_data_$ct_id&_type=content_data&content_type_id=$ct_id&id=" . $cd1->id,
        id                                  => $cd1->id,
        blog_id                             => $blog_id,
        content_type_id                     => $ct_id,
        save_revision                       => 1,
...
subtest 'update content_data (change categories field)' => sub {
    my $cd1 = MT->model('content_data')->load({ blog_id => $blog_id, label => 'cd1' })
        or die;
    my $app = MT::Test::App->new('MT::App::CMS');
    $app->login($author);
    $app->post_ok({
        ...
        save_revision                       => 1,
        data_label                          => 'cd1',
        'content-field-' . $cf_category->id => $category2->id,
        'category-' . $cf_category->id      => $category2->id,
        'date-' . $cf_datetime->id          => '2018-10-31',
        'time-' . $cf_datetime->id          => '00:00:00',
        authored_on_date                    => '2017-05-30',
        authored_on_time                    => '16:36:00',
