...
sub create_preview_file_path {
    ...
    my $preview_file_name
        = 'mt-preview-' . MT::Util::perl_sha1_digest_hex($data);

    my $preview_date
        = Time::Piece->strptime( scalar $app->param('authored_on_date'),
        '%Y%m%d' );
...
subtest 'Delete preview file. (PreviewInNewWindow is 0)' => sub {
    ...
    subtest
        'Delete preview file when navigating from preview page to edit page.'
        => sub {
        ...
        my ( $preview_file_path, $preview_file_name )
            = &create_preview_file_path($app_content_preview);

        is( $file_manager->exists($preview_file_path),
            1, 'Successful creation of preview file.' );
        ...
        ok( !$file_manager->exists($preview_file_path),
