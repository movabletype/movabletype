...
subtest 'Migrate DataAPIDisableSite as disabled' => sub {
    my $website = MT::Test::Permission->make_website(name => 'my website');
    my $terms = {
        plugin => 'DataAPI',
        key    => $key_prefix . $website->id,
    };
    my $pd = MT->model('plugindata')->load($terms);
    $pd->remove; # Force drop plugindata for pseudo initial state 
    _update_data_api_disable_site($website, 1); # Add to DataAPIDisableSite

    MT::Test::Upgrade->upgrade(from => 7.0045);

    $pd = MT->model('plugindata')->load($terms);
    is($pd->data->{enable_data_api}, 0);
};
