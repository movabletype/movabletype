...
sub start_element {
    ...
    if ($self->{start}) {
        die MT->translate('The uploaded file was not a valid Movable Type exported manifest file.')
    ...
    if (my $current = $self->{current}) {
        ...
        if (MT::BackupRestore::NS_MOVABLETYPE() eq $ns) {
            ...
            unless ($class) {
                push @{ $self->{errors} }, MT->translate('[_1] is not a subject to be imported by Movable Type.', $name);
            } else {
                if (!$self->{current_class} or $self->{current_class} ne $class) {
                    if (my $c = $self->{current_class}) {
                        ...
                        $callback->(
                            $state . " " . MT->translate("[_1] records imported.", $records),
                            $c->class_type || $c->datasource
                        );
                    }
                    $self->{records}       = 0;
                    $self->{current_class} = $class;
                    my $state = MT->translate('Importing [_1] records:', $class);
                    $callback->($state, $name);
                    $self->{state} = $state;
                }
                ...
                if ($class =~ /^MT::Asset/ || $class eq 'MT::Blog' || $class eq 'MT::Website') {
                    my $key = $class =~ /^MT::Asset/ ? 'file_path' : 'upload_path';
                    if ($column_data{$key}) {
                ...
                if ('author' eq $name) {
                    ...
                    if ($obj) {
                        if (UNIVERSAL::isa(MT->instance, 'MT::App') && ($obj->id == MT->instance->user->id)) {
                            MT->log({
                                ...
                                level    => MT::Log::INFO(),
                                metadata => 'Permissions and associations have been imported.',
                                class    => 'system',
                                category => 'restore',
                            });
                            ...
                            MT->log({
                                ...
                                level    => MT::Log::INFO(),
                                metadata => 'Permissions and associations have been imported as well.',
                                class    => 'system',
                                category => 'restore',
                            });
                            ...
                            delete $column_data{userpic_asset_id} if exists $column_data{userpic_asset_id};

                            my $success = $obj->restore_parent_ids(\%column_data, $objects);
                            if ($success) {
                                my %realcolumns = map { $_ => _decode(delete($column_data{$_})) } @{ $obj->column_names };
                    ...
                    if (!$column_data{blog_id}) {
                        ...
                        if ($obj) {
                            if ($obj->created_on == $column_data{created_on}) {
                                ...
                                {
                                    my ($name, $path) = fileparse($column_data{file_path});
                                    $column_data{file_path} = File::Spec->catfile($path, $middle_path, $name);
                                }

                                {
                                    my ($path, $name) = ($column_data{url} =~ m{(.*/)(.*)}s);
                                    $column_data{url} = $path . $middle_path . '/' . $name;
                                }
                            }
                        }
                        MT::Util::Log->info('   End import   ' . $class);
                    ...
                    my $new_ct = $objects->{ 'MT::ContentType#uid:' . $column_data{ct_unique_id} };
                ...
                unless ($obj->id) {
                    ...
                    if ($success || (!$success && 'blog' eq $name)) {
                        ...
                        my %realcolumn_data = map { $_ => _decode($column_data{$_}) }
                            grep { !exists($metacolumns{$_}) } keys %column_data;
                        ...
                        # Restore modulesets
                        if ('template' eq $name && $obj->type eq 'widgetset') {
        ...
        if (my $text_data = delete $self->{current_text}) {
            ...
            if (exists($defs->{$column_name})) {
                if ('blob' eq $defs->{$column_name}->{type}) {
                    ...
                    if (substr($text, 0, 4) eq 'SERG') {
                        my $ser_ver = MT::Serialize->serializer_version($text);
                        if ($ser_ver == 3) {
            ...
            if ('tag' eq $name) {
                ...
                if (my $tb = $class->load($term)) {
                    ...
                    my $records = $self->{records};
                    $self->{callback}->(
                        $self->{state} . " " . MT->translate("[_1] records imported...", $records),
                        $data->{LocalName}) if $records && ($records % 10 == 0);
                ...
                if ($obj->blog_id == 0) {
                    my $field = $class->exist({ blog_id => 0, basename => $obj->basename });
                    $exists = 1 if $field;
                }
            } elsif ('role' eq $name) {
                ...
                if ($role) {
                    ...
                    if ($old_perms eq $cur_perms
                        && !MT::BackupRestore::ContentTypePermission->has_permission($old_perms))
                    {
                ...
                if ($obj->key !~ /^configuration:blog:(\d+)$/i) {
                    if (my $obj = MT->model('plugindata')->load({ key => $obj->key, plugin => $obj->plugin })) {
                        $exists = 1;
                        $self->{callback}->(MT->translate(
