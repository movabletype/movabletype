    ...
    if ( $self->{start} ) {
        ...
        if ( $schema != $self->{schema_version} ) {
            ...
            my $message = MT->translate(
                ...
                MT::I18N::encode_text( MT::I18N::utf8_off($schema), 'utf-8' ),
                $self->{schema_version}
            );
            MT->log(
                {   message  => $message,
    ...
    if ( my $current = $self->{current} ) {
        ...
        if ( MT::BackupRestore::NS_MOVABLETYPE() eq $ns ) {
            ...
            unless ($class) {
                ...
                my %column_data = map {
                    $attrs->{$_}->{LocalName} => MT::I18N::encode_text(
                        MT::I18N::utf8_off( $attrs->{$_}->{Value} ), 'utf-8' )
                } keys(%$attrs);
                my $obj;
                if ( 'author' eq $name ) {
...
sub end_element {
    ...
    if ( my $obj = $self->{current} ) {
        if ( my $text_data = delete $self->{current_text} ) {
            ...
            my $old_id = $obj->id;
            unless ( ( ( 'author' eq $name ) || ( 'template' eq $name ) )
                && ( exists $self->{loaded} ) )
            {
