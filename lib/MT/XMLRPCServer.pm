...
sub getUserInfo {
    ...
    die _fault( MT->translate("Invalid login") ) unless $author;
    my ( $fname, $lname )
        = map { encode_text( $_, undef, 'utf-8' ) } split /\s+/,
        $author->name;
    $lname ||= '';
        ...
        if ( $class eq 'blogger' ) {
            ...
            unless ($titles_only) {
                ...
                $row->{permaLink} = SOAP::Data->type( string => $link ),
                    $row->{mt_basename} = SOAP::Data->type(
                    string => _encode_text_for_soap(
                ...
                $row->{mt_text_more} = SOAP::Data->type(
                    string => _encode_text_for_soap(
                        $entry->text_more, undef, 'utf-8'
                    )
                );
                $row->{mt_excerpt} = SOAP::Data->type(
                    string => _encode_text_for_soap(
                        $entry->excerpt, undef, 'utf-8'
                    )
                );
                $row->{mt_keywords} = SOAP::Data->type(
                    string => _encode_text_for_soap(
...
sub _get_entry {
    ...
    {   dateCreated => SOAP::Data->type( dateTime => $co ),
        ...
        mt_keywords => SOAP::Data->type(
            string =>
                _encode_text_for_soap( $entry->keywords, undef, 'utf-8' )
        ),
