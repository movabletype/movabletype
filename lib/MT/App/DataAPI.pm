...
'Do not have permission to retrieve the list of activity logs.',
        ...
        },
        {
            ...
            error_codes => { 403 => 'Do not have permission to reset logs.', },
        },
        {
            id      => 'export_logs',
            ...
            error_codes => { 403 => 'Do not have permission to export logs.', },
        },
    ...
    $app->_register_core_callbacks(
        {
            ...
            # category callbacks
            $pkg
              . 'view_permission_filter.category' => "${pfx}Category::can_view",
            ...
            # template callbacks
            $pkg
              . 'list_permission_filter.template' => "${pfx}Template::can_list",
            $pkg
              . 'view_permission_filter.template' => "${pfx}Template::can_view",
            ...
            $pkg . 'post_save.widget' => '$Core::MT::CMS::Template::post:save',
            ...
            $pkg . 'pre_save.widgetset' => '$Core::MT::CMS::Template::pre_save',
            $pkg
              . 'post_save.widgetset' => '$Core::MT::CMS::Template::post:save',
            ...
            $pkg
              . 'save_filter.templatemap' => "${pfx}TemplateMap::save_filter",
            ...
            $pkg
              . 'post_delete.templatemap' => "${pfx}TemplateMap::post_delete",

            # plugin callbacks
            $pkg . 'list_permission_filter.plugin' => "${pfx}Plugin::can_list",
            $pkg . 'view_permission_filter.plugin' => "${pfx}Plugin::can_view",
        }
    );
...
sub send_cors_http_header {
    ...
    $app->set_header(
        'Access-Control-Expose-Headers' => $config->DataAPICORSExposeHeaders );
}
...
sub api {
        ...
        || UNIVERSAL::can( $response, 'to_resource' ) )
    {
        ...
        my $resource =
          $app->object_to_resource( $response,
