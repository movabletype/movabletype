...
sub prepare_context {
    ...
    $ctx->{current_archive_type} = $app->param('archive_type')
        if ( $app->param('archive_type') );
    ...
    $ctx->var( 'datebased_archive', 1 )
        if (
        $app->param('archive_type')
        && (   $app->param('archive_type') =~ /Daily/i
            || $app->param('archive_type') =~ /Weekly/i
            || $app->param('archive_type') =~ /Monthly/i
            || $app->param('archive_type') =~ /Yearly/i )
        );
