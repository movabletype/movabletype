...
<MTEntries>
    ...
    <mt:if name="__last__">
    <div class="view-all"><a href="<mt:var name="script_url">?__mode=list_entry&amp;filter=exacttag&amp;filter_val=<mt:var name="tag" escape="url"><mt:if name="blog_id">&amp;blog_id=<$mt:var name="blog_id"$></mt:if>"><__trans phrase="View all entries tagged &lsquo;[_1]&rsquo;" params="<mt:var name="tag" escape="html">"></a></div>
    </mt:if>
<MTElse>
    <mtapp:statusmsg    
        id="warning"
            ...
            _by_date => {
                label   => sub {
                    ...
                    $to = MT::Util::format_ts($format, $to . '000000', undef, MT->current_language)
                        if $to;
                    if ($from && $to) {
                        return $app->translate('Entries posted between [_1] and [_2]', $from, $to);
                    } elsif ($from) {
                        return $app->translate("Entries posted since [_1]", $from);
                    } elsif ($to) {
                        return $app->translate("Entries posted on or before [_1]", $to);
                    }
                },
                handler => sub {
                    my ( $terms, $args ) = @_;
...
sub list_associations {
    ...
    return $app->build_page( 'list_association.tmpl', $param );
}
...
sub gen_dashboard_stats {
    ...
    my $static_path = $app->config('StaticWebPath');
    ...
    my $comment_file =
      'blog-' . $blog_id . ',user-' . $user_id . ',comment.xml';
    $param->{comment_stat_url} = $static_path . 'support/' . $comment_file;
    my $comment_path = File::Spec->catfile( $static_file_path,
        'support', $comment_file );

    my $entry_file = 'blog-' . $blog_id . ',user-' . $user_id . ',entry.xml';
    $param->{entry_stat_url} = $static_path . 'support/' . $entry_file;
    my $entry_path = File::Spec->catfile( $static_file_path,
        'support', $entry_file );
...
sub build_menus {
    ...
    foreach my $id (@top_ids) {
        ...
        next if $menu->{requires_blog} && !$blog_id;
        ...
        foreach my $sub_id (@sub_ids) {
            $menus->{allowed} = 0
              if $menus->{$sub_id}{requires_blog} && !$blog_id;
        ...
        if (@sub) {
            ...
            foreach my $sub (@sub) {
                ...
                $sub->{allowed} = 0 if $sub->{requires_blog} && !$blog_id;
                if ( $sub->{allowed} ) {
...
sub dashboard {
    ...
    my $q             = $app->param;
    my $blog_class    = $app->model('blog');
    my $perm_class    = $app->model('permission');
    my $entry_class   = $app->model('entry');
    my $comment_class = $app->model('comment');
    my $author        = $app->user;
    ...
    $param{publish_count} = MT::Entry->count( { author_id => $author->id, } );
...
sub edit_object {
    ...
    if ($id) {    # object exists, we're just editing it.
        ...
        if ( ( $type eq 'entry' ) || ( $type eq 'page' ) ) {
                    ...
                    push @tmpl_ids,
                      {
                        label    => $dtmpl->{label},
                        key      => $dtmpl->{key},
                        selected => $dtmpl->{key} eq ( $obj->identifier || '' ),
                      };
                }
            }
                ...
                || $obj_type eq 'archive' )
            {
                ...
                $param{template_map_loop} = $maps if @$maps;
            }
        }
        elsif ( $type eq 'blog' ) {
            ...
            if ( $output eq 'cfg_prefs.tmpl' ) {
                ...
                $param{use_comment_confirmation} =
                  ...
                  : $cfg->UseCommentConfirmationPage;
    ...
    if ( ( $type eq 'entry' ) || ( $type eq 'page' ) ) {
        ...
        $type = 'entry' if $type eq 'page';    # for template name
        ...
        if ( !$param{id} ) {
            if ( $param{site_path} ) {
                $param{site_path} =
                  File::Spec->catdir( $param{site_path}, 'WEBLOG-NAME' );
            }
            else {
    ...
    # If not yet defined, set the site_url to the config default, if one exists.
        ...
        if ( !$param{id} ) {
            if ( $param{site_url} ) {
                ...
                $param{suggested_site_url} .= 'WEBLOG-NAME/';
            }
        }
    }
    elsif ( $type eq 'author' ) {
    ...
    for my $p (@p) {
        if ( $p =~ m/^(.+?):(\d+)$/ ) {
            ...
            if ( ( lc($p) eq 'Advanced' ) || ( lc($p) eq 'default' ) ) {
                ...
                if ( $p eq 'Default' ) {
                    ## default fields which can be disabled
                    foreach my $def (
...
sub set_object_status {
    ...
    return $app->error( $app->translate('Invalid type') )
      unless ( $type eq 'author' ) || ( $type eq 'group' );
        ...
        if ($most_recent) {
            ...
            if ( my $ts = $most_recent->created_on ) {
                $row->{most_recent_time_formatted} =
                  format_ts( "%Y-%m-%d %H:%M:%S", $ts );
                $row->{most_recent_formatted} = format_ts( "%Y.%m.%d", $ts );
    ...
    for my $list_key ( sort keys %list ) {
        ...
        if ( my $plugin = $profile->{object} ) {
            ...
            my $row = {
                ...
                plugin_id            => $id,
                plugin_compat_errors => $plugin->{registry}{compat_errors},
            };
            $row->{plugin_tags} = listify( $profile->{tags} )
              if $profile->{tags};
    ...
    if ( $filter_col && ( my $val = $q->param('filter_val') ) ) {
            ...
            || $filter_col eq 'entry_id' )
        {
            ...
            my $tb = MT::Trackback->load(
                {
                    $blog_id ? ( blog_id => $blog_id ) : (), $filter_col => $val
                }
            );
            if ($tb) {
...
sub build_entry_table {
    ...
    while ( my $obj = $iter->() ) {
        ...
        $row->{category_name} =
          $obj->category
          ? (
            $type eq 'page' ? $obj->category->basename : $obj->category->label )
          : '';
    ...
    $app->load_list_actions( $type, $param->{entry_table}[0] )
...
sub list_category {
    ...
    $param{sub_object_label} =
      $app->translate( $type eq 'folder' ? 'Subfolder' : 'Subcategory' );
    $param{object_label}        = $class->class_label;
...
sub cfg_archives {
    ...
    my @maps = $app->model('templatemap')->load({ blog_id => $blog_id });
    my %maps = map { $_->archive_type => 1 } @maps;
    my %ats_enabled = map { $_ => 1 } split(',', $blog->archive_type);
    my @data;
    my $ats = $app->registry('archive_types');
    foreach my $at ( keys %$ats) {
        my $archive_label = $ats->{archive_label};
        $archive_label = $at unless $archive_label;
        $archive_label = $archive_label->() if 'CODE' eq ref($archive_label);
        my $data =
          {
            archive_type_translated   => $archive_label,
            archive_type              => $at,
            ...
            available                 => exists $maps{$at} ? 1 : 0,
          };
        if ( exists $ats_enabled{$at} ) {
            $data->{enabled} = 1;
            $data->{entry}   = 1 if $ats->{$at}->entry_class eq 'entry';
        }
        push @data, $data;
    }
    @data = sort { archive_type_sorter( $a, $b ) } @data;
    $param{blog_archive_types} = \@data;
...
sub cfg_archives_save {
    ...
    my @ats;
    for my $p (@p) {
        if ( $p =~ /^archive_type_([\w\-]+)$/ ) {
            my $at     = $1;
            push @ats, $1 if $q->param($p);
        }
    }
    if (@ats) {
...
sub cfg_system_feedback {
    ...
    $param{use_confirmation} = $cfg->UseCommentConfirmationPage;
...
sub save_cfg_system_feedback {
    ...
    $cfg->UseCommentConfirmationPage( $app->param('use_confirmation') || 0, 1 );
    if ( my $key = $app->param('captcha_provider') ) {
        if ( 'none' eq $key ) {
            $cfg->CaptchaProvider( undef, 1 );
        }
        else {
            my $cp = MT->captcha_provider($key);
            $cfg->CaptchaProvider( $cp->{key}, 1 );
        }
    }
...
sub start_export {
    ...
    my $perms = $app->permissions;
    return $app->redirect_to_dashboard( permission => 1 )
      unless $perms && $perms->can_edit_config;
...
sub dialog_grant_role {
    ...
    if ( $app->param('search') || $app->param('json') ) {
        ...
        if ( $type && $type eq 'author' ) {
            if ( !$app->param('offset') ) {
                my $data = $params->{object_loop} ||= [];
                unshift @$data, $pseudo_user_row;
            }
        }
        return $app->build_page( "include/listing_panel.tmpl", $params );
    }
    else {
...
sub dialog_post_comment {
    ...
    my $parent = MT::Comment->load($parent_id)
