...
sub list {
    ...
    $param{page_actions} = $app->page_actions('list_author');
    ...
    my $hasher = sub {
        ...
        my @roles = MT::Role->load(
            ...
            {   join => MT::Association->join_on(
                    'role_id',
                    { author_id => $row->{id}, blog_id => $blog_id },
                    { unique    => 1 }
                )
            }
        );
        ...
        foreach my $role (@roles) {
            ...
            my $is_removable = !$role->has('administer_blog')
                || $app->can_do('remove_administer_member');
            push @role_loop,
                {
                role_name    => $role->name,
                role_id      => $role->id,
...
PERMCHECK: {
        ...
        last PERMCHECK
            if $blog_id
                ...
                ->can_do('access_to_blog_association_list');
        last PERMCHECK
...
sub save_role {
    ...
    $app->can_do('save_role') or return $app->errtrans("Permisison denied.");
