...
sub dashboard {
    ...
    if ( defined $blog_id && !$user->has_perm($blog_id) ) {
        ...
        if ( $blog && $blog->is_blog ) {
            my @current
                = grep { $_ != $blog_id } @{ $user->favorite_blogs || [] };
            $user->favorite_blogs( \@current );
        }
        elsif ( $blog && !$blog->is_blog ) {
            my @current
                = grep { $_ != $blog_id } @{ $user->favorite_websites || [] };
            $user->favorite_websites( \@current );
        }
        $user->save;

        return $app->return_to_user_dashboard( redirect => 1,
            permission => 1 );
    }
...
sub new_version_widget {
    ...
    push @{ $param->{feature_loop} ||= [] },
        ...
        feature_description => MT->translate(
            'Create and apply a theme to change templates, categories, folders and custom fields.'
        ),
        },
        {
        feature_label       => MT->translate('Website Management'),
        feature_description => MT->translate(
            'Manage multiple blogs for each website. Now, it\'s much easier to create a portal with MultiBlog.'
        ),
        },
        {
        feature_label       => MT->translate('Revision History'),
        feature_description => MT->translate(
            'The revision history for entries and templates protects users from unexpected modification.'
        ),
        },
        {
        feature_label => MT->translate('Movable Type Online Manual'),
        feature_url =>
            MT->translate('http://www.movabletype.org/documentation/'),
        feature_description => MT->translate(
            'Whether you\'re new to Movable Type or using it for the first time, learn more about what this tool can do for you.'
        ),
        };
}
