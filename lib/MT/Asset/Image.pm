...
sub extensions {
    my $pkg = shift;
    return $pkg->SUPER::extensions( [ qr/gif/i, qr/jpe?g/i, qr/png/i, qr/bmp/i, qr/ico/i ] );
}
...
sub has_gps_metadata {
    ...
    return 0 if lc( $asset->file_ext ) !~ /^(jpe?g)$/;
...
sub remove_gps_metadata {
    ...
    return 1 if lc( $asset->file_ext ) !~ /^(jpe?g)$/;
