...
#
# $Id: FTP.pm 3531 2009-03-12 09:11:52Z fumiakiy $
...
sub mkpath {
    ...
    $fmgr->{ftp}->mkdir($path, 1)
        or return $fmgr->error(MT->translate("Creating path '[_1]' failed: [_2]", $path, $@));
    1;
}
...
sub WRITE {
    my $str = substr($_[1], $_[3] ? $_[3] : 0, $_[2] ? $_[2] : length($_[1]));
    $_[0]->{buf} .= $str;
    length($str);
}
sub BINMODE { 1 }
