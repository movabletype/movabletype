...
sub config_tmpl {
    ...
    $plugin->load_tmpl(
        'web_service_config.tmpl',
        {
            ...
            (   $flickr_configured
                ? ( flickr_configured_consumer_key => $config->{flickr_consumer_key},
                    flickr_configured_consumer_secret => $config->{flickr_consumer_secret},
                    )
                : ()
            ),
...
sub flickr_oauth_request {
    ...
    _set_session_token_data( $app, 'flickr', { request_token_secret => $request_token_secret } );
...
sub flickr_oauth_success {
    ...
    my $flickr_token = _get_session_token_data($app, 'flickr');
    my $request_token_secret = $flickr_token && $flickr_token->{request_token_secret}
    ...
    #$ua->ssl_opts( verify_hostname => 0 );
    my $http_hdr = HTTP::Headers->new( 'Authorization' => $request->to_authorization_header );
    my $http_req = HTTP::Request->new( $request_method,$access_token_url,$http_hdr );
    my $res = $ua->request( $http_req );
    my $token_data = {};
    if ($res->is_success) {
