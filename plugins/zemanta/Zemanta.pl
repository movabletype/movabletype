...
<script id="zemanta-loader" type="text/javascript" src="http://fstatic.zemanta.com/plugins/movabletype/4.x/loader.js"></script>
<!-- End Zemanta scripts -->
END
}
...
sub entry_prefs_first_run {
	...
	unless( $have_default || !$blog_id ) {
		...
		$perm->entry_prefs("zemanta,basic");
		$perm->save();
	}
...
sub edit_entry {
	...
	# We place our setvarblock right before the header include.
	...
	my $setvar_node = $tmpl->createElement('setvarblock', 
					{ name => 'js_include', append => 1 });

	$setvar_node->innerHTML(javascript_block($api_key, $plugin->envelope));

	$tmpl->insertBefore($setvar_node, $header_node);
	...
	$setvar_node = $tmpl->createElement('setvarblock', 
					{ name => 'related_content', prepend => 1 });
	
	my $div_class = $param->{disp_prefs_show_zemanta} ? '' : ' class="hidden"';
	$setvar_node->innerHTML("<div id=\"zemanta-field\"$div_class></div>");

	$tmpl->insertAfter($setvar_node, $header_node);
...
sub cfg_entry {
	...
	# Enable Zemanta in "Display options" for all blogs and users on first
	# run of the plugin.
	if( $plugin->get_config_value("first_run", "system") ) {
		entry_prefs_first_run($param);
		$plugin->set_config_value("first_run", "", "system");
	}

	# Add a check-box into the "Default Editor Fields" section.
	my $disp_prefs_node = $tmpl->getElementById('entry_prefs');
				
	my $checked = $param->{disp_prefs_show_zemanta} ? ' checked="checked"' : '';

	my $disp_prefs_html = $disp_prefs_node->innerHTML();
	$disp_prefs_html =~ s#</ul>#<li><input 	type="checkbox" name="custom_prefs" 
						id="custom-prefs-zemanta"
						value="zemanta"
						class="cb"
						$checked/>
					<label for="custom-prefs-zemanta">Zemanta</label></li></ul>#;
	$disp_prefs_node->innerHTML($disp_prefs_html);
}
