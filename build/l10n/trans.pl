...
do {
    ...
    if ( ( $tmpl ne $ARGV ) || eof() ) {
        ...
        if ( $tmpl =~ m|plugins/([\-\w]+)/\w+| ) {
            my $plugin = $1;
            eval { unshift @INC, "plugins/$plugin/lib"; };
            eval { require "plugins/$plugin/lib/$plugin/L10N/$lang.pm"; };
            if ($@) {
                ...
                unless ( $phrase{ $args{phrase} } ) {
                    ...
                    if ($trans) {
                        ...
                        printf "\t$qs%s$qe => $qs%s$qe,\n", $args{phrase},
                            $trans
                            ; # Print out translation if there was an existing one
                    }
                    else {
                       ...
                       #                        $trans =~ s/([^\\]?)'/$1\\'/g;
                        my $reason
                            ...
                            ? 'Case'
                            : 'New'
                            ;  # Really new translation or just different case
                        utf8::decode($trans) if !utf8::is_utf8($trans);
                        printf
                            "\t$qs%s$qe => $qs%s$qe, # Translate - $reason\n",
                            $args{phrase}, $trans
                            ; # Print out translation if there was an existing one based on the lowercase string, empty otherwise
                    }
                }
            }
        }
        while ( $text
            =~ /(?:translate|errtrans|trans_error|trans|translate_escape|maketext)\s*\(((?:\s*(?:"(?:[^"\\]+|\\.)*"|'(?:[^'\\]+|\\.)*'|q{(?:[^}\\]+|\\.)*})\s*\.?\s*){1,})[,\)]/gs
            )
        {
            my ( $msg, %args );
            my $p = $1;
            ...
            if ($trans) {
                utf8::decode($trans) if !utf8::is_utf8($trans);
                printf "\t$q%s$q => $q%s$q,\n", $args{phrase}, $trans
                    ; # Print out the translation if there was an existing one
            }
            else {
                $trans = $lconv{ lc $args{phrase} } || '';
                my $reason
                    = $trans
                    ? "Case"
                    : "New";    # New translation, or just different case?
                utf8::decode($trans) if !utf8::is_utf8($trans);
                printf "\t$q%s$q => $q%s$q, # Translate - $reason\n",
                    $args{phrase}, $trans
                    ; # Print out the translation if there was an existing one based on the lowercase string, else empty
            }
        }
        while ( $text
            =~ /\s*(?:["'])?label(?:_plural)?(?:["'])?\s*=>\s*(["'])(.*?)([^\\])\1/gs
            )
        {
            next if $2 =~ /^$1/;
            my ( $msg, %args );
        ...
        if ( $tmpl =~ /(services|streams)\.yaml$/ ) {
            while (
                $text =~ /\s*(?:description|ident_hint|label|name):\s*(.+)/g )
            {
                my ( $msg, %args );
                my $trans = '';
                $args{phrase} = $1;
