...
my $index_only = 0;             # only create an index
...
$index_only = 0;        # only create an index (table of contents)
...
<tr><td class="block" width="100%" bgcolor="#CCCCCC">
...
END_OF_HEAD
    ...
    print HTML $index;
    print HTML "-->\n" unless $doindex;
    print HTML "<hr size=\"1\" color=\"#CCCCCC\">\n" if $doindex and $index;
	...
	$_ = $poddata[$i];
        ...
        if ($saved_code && /^\S/) {
            $saved_code =~ s!^\s{4}!!gm;
            #my $lines = ($saved_code =~ tr/\n//);
            #$lines = 10 if $lines > 10;
            #$lines = 2 if $lines <= 1;
            #print HTML qq(<br /><br /><textarea rows="$lines" cols="80">$saved_code</textarea><br />);
	    print HTML "<pre>$saved_code</pre>";
            $saved_code = '';
        }
...
sub parse_command_line {
	...
	$opt_title,$opt_verbose,$opt_index_only);
    ...
    $index_only = $opt_index_only if defined $opt_index_only;
...
sub process_head {
    ...
    print HTML "<h$level><a name=\"$name\"></a>$convert</h$level>\n";
}
...
sub emit_item_tag($$$){
	...
	my $name = 'item_' . $item;
	print HTML qq{<a name="$name"></a>}, process_text( \$otext );
    }
...
sub process_pre {
    ...
    $rest =~ s{
		 ...
		 if ( defined $pages{$2} ){	# is a link
		     qq($1<a href="$pages{$2}">$2</a>);
		 } elsif (defined $pages{dosify($2)}) {	# is a link
		     qq($1<a href="$pages{dosify($2)}">$2</a>);
		 } else {
		     "$1$2";
		 }
	      }xeg;
...
sub process_text1($$;$$){
	...
	$res = '<i>' . process_text1( $lev, $rstr ) . '</i>';
